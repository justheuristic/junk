...
def calculating_XTX(args):
    ...
    '''
    X = torch.load(args.input_loading_dir,
...
def compress(args,run):
    quantized_layer = QuantizedWeight(
        weight_shape=reference_weight.shape, num_codebooks=args.num_codebooks,
        nbits_per_codebook=args.nbits_per_codebook,
        out_group_size=args.out_group_size, in_group_size=args.in_group_size, 
        device=device,
        init_kmeans=args.kmeans_init, reference_weight=reference_weight, alpha=1.0, verbose=True
    )
    ...
    for epoch in range(args.num_epochs):
        ...
        if (epoch + 1) % args.beam_search_epochs == 0:
            ...
            quantized_layer.requantize_(
                ...
                beam_size=args.beam_size if (epoch + 1) % 1000 != 0 else big_beam_size,
            ...
            if args.sparsity_regularizer != 0:
                ...
                if (epoch + 1) % args.big_beam_search_epochs == 0:
                    ...
                    if args.in_group_size>1 and args.out_group_size>1:
                        curr_avg_bits  = calc_avg_bits(args.num_codebooks, 1, mean_code_nbits,
                                            args.nbits_per_codebook, reference_weight.shape[0],reference_weight.shape[1])
